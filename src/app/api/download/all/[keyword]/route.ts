...
export async function GET(
  ...
  if (!filesData || filesData.length === 0) {
    return NextResponse.json({ error: 'No files found for this keyword or keyword does not exist.' }, { status: 404 });
  }
  ...
  try {
    ...
    for (const file of filesData) {
      ...
      const dummyContent = `This is the content for ${file.fileName}.\nUploaded at: ${file.uploadedAt.toISOString()}\nSize: ${file.size} bytes.\nContent Type: ${file.contentType}`;
    ...
    return new NextResponse(zipBuffer, { status: 200, headers: responseHeaders });
