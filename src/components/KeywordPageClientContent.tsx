...
export default function KeywordPageClientContent({ initialFilesData, keyword }: KeywordPageClientContentProps) {
  ...
  const isMockFileDisplayed = currentFiles.length === 1 && currentFiles[0].fileName === 'sample-demonstration-file.pdf' && (!initialFilesData || initialFilesData.length === 0);
  ...
  useEffect(() => {
    if (initialFilesData === null || initialFilesData.length === 0) {
      ...
      setCurrentFiles(initialFilesData.map(file => ({...file, uploadedAt: new Date(file.uploadedAt)})));
    }
  ...
  const handleFetchFilesData = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setCurrentFiles(data.map(file => ({...file, uploadedAt: new Date(file.uploadedAt)})));
      }
    } catch (e: any) {
  ...
  const handleDeleteAllFiles = async () => {
    ...
    try {
      if (isMockFileDisplayed && (!initialFilesData || initialFilesData.length === 0)) {
        setCurrentFiles([]);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-3 sm:gap-4">
          ...
          <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
            <Button
              ...
              disabled={isActionLoading['downloadAll'] || currentFiles.length === 0 || (isMockFileDisplayed && (!initialFilesData || initialFilesData.length === 0))}
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  disabled={isActionLoading['deleteAll'] || currentFiles.length === 0 || (isMockFileDisplayed && (!initialFilesData || initialFilesData.length === 0))}
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    {(isMockFileDisplayed && currentFiles[0]?.fileName === 'sample-demonstration-file.pdf') ? " This will clear the sample file from view." : " This cannot be undone."}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    ...
                    <CardContent className="p-3 sm:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3">
                      ...
                      <div className="flex gap-2 mt-2 sm:mt-0 flex-shrink-0 w-full sm:w-auto">
                        <Tooltip>
                          ...
                          <TooltipContent side="top">Download {file.fileName}</TooltipContent>
                        </Tooltip>
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button variant="outline" size="sm" disabled={isActionLoading[file.fileName]} className="flex-1 sm:flex-none text-destructive border-destructive hover:bg-destructive/10">
                                  {isActionLoading[file.fileName] ? <Loader2 className="h-4 w-4 animate-spin sm:mr-1.5" /> : <Trash2 className="h-4 w-4 sm:mr-1.5" />}
                                  <span className="hidden sm:inline">Delete</span>
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent side="top">Delete {file.fileName}</TooltipContent>
                            </Tooltip>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
                            <AlertDialogHeader>
